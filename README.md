...
`RUN_MUNICH_TOOLKIT.bat`는 통합 대시보드를 항상 8501 포트에서 실행합니다. 독일어 STT 단독 앱은 포트 충돌을 피하기 위해 8502 포트를 사용합니다.
...
대시보드 자체는 가볍게 실행되지만, 독일어 영상 자막 기능과 캡션 한국어 번역 기능은 Whisper/Transformers가 필요합니다.
필요한 패키지가 없으면 대시보드 화면에 설치 버튼이 표시됩니다.

- `photo collector`: 캡션 한국어 번역 패키지 설치
- `german subtitle translator`: 자막/번역 패키지 설치
